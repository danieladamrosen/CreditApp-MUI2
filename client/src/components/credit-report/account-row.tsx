...
export function AccountRow({
  ...
  console.log("🐞 [DEBUG-SNAPSHOT]", {
    id: account?.['@CreditLiabilityID'],
    creditor: account?._CREDITOR?.['@_Name'],
    keys: Object.keys(account || {})
  });

  // DEBUG-BUREAU-STRUCTURE  (remove after test)
  console.log('🔍 RAW KEYS FOR', account['@CreditLiabilityID'], ':', Object.keys(account));
  ['transUnion', 'equifax', 'experian'].forEach(bKey => {
    console.log('   ', bKey, 'exists →', !!account[bKey], '→', account[bKey]);
  });
  ...
  const checkFormCompletionAndShowArrow = (
    ...
    // Don't show arrow during typing animation (matching inquiries behavior)
  ...
  // useEffect to monitor form completion and show arrow automatically
  useEffect(() => {
...
Due to these reporting inaccuracies and regulatory violations, I am requesting the permanent deletion of this tradeline from my credit report.`;
  ...
  // Use authentic data from JSON - only show data for bureaus that actually report this account
  const getBureauData = (account: any, bureau: 'TU' | 'EQ' | 'EX') => {
    if (!account) {
      console.log(`⚠️ getBureauData: No account object provided for bureau ${bureau}`);
      return { reporting: false };
    }

    // Map bureau codes to account property names
    const bureauPropertyMap = {
      'TU': 'transUnion',
      'EQ': 'equifax', 
      'EX': 'experian'
    };
    
    const bureauProperty = bureauPropertyMap[bureau];
    const bureauData = account[bureauProperty];
    
    // Only return false if bureau data is completely missing or empty
    if (!bureauData || (typeof bureauData === 'object' && Object.keys(bureauData).length === 0)) {
      console.log(`📋 getBureauData: No ${bureauProperty} data found for bureau ${bureau}`);
      return { reporting: false };
    }
    
    // Extract bureau-specific data with comprehensive null safety
    const balanceRaw = bureauData['@_CurrentBalance'] || 
                      bureauData['@_UnpaidBalanceAmount'] || 
                      bureauData['@_AccountBalance'] || 
                      bureauData['@_HighCreditAmount'] || '0';
    const realBalance = parseInt(String(balanceRaw)) || 0;
    
    const realStatus = bureauData._CURRENT_RATING?.['@_Code'] || 
                      bureauData['@_AccountRating'] || 
                      bureauData['@_AccountStatusType'] || 
                      bureauData['@_PaymentRating'] || 'Unknown';
    
    const realOpenDate = bureauData['@_DateOpened'] || 
                        bureauData['@_AccountOpenedDate'] || 
                        bureauData['@_DateAccountOpened'] || 'N/A';
    
    const realLastUpdated = bureauData['@_AccountReportedDate'] || 
                           bureauData['@_AccountBalanceDate'] || 
                           bureauData['@_DateAccountUpdated'] || 
                           bureauData['@_LastActivityDate'] || 'N/A';
    
    // Log when fallback defaults are used for data quality tracking
    if (balanceRaw === '0' || !balanceRaw) {
      console.log(`💰 Using default balance (0) for ${bureau} - no balance data found`);
    }
    if (realStatus === 'Unknown') {
      console.log(`📊 Using default status (Unknown) for ${bureau} - no status data found`);
    }
    if (realOpenDate === 'N/A') {
      console.log(`📅 Using default open date (N/A) for ${bureau} - no date data found`);
    }
    if (realLastUpdated === 'N/A') {
      console.log(`🔄 Using default last updated (N/A) for ${bureau} - no update date found`);
    }
    ...
    return {
      ...
      openDate: realOpenDate,
      isNegative: false, // Will be determined by isCurrentlyNegative function
      lastUpdated: realLastUpdated,
      reportingBureau: bureau,
    };
  };



  // Single-parameter getBureauData function for bureau rendering loop
  function getBureauDataByKey(account: any, key: 'TU' | 'EQ' | 'EX') {
    ...
    const openDate = account.AccountOpenedDate || 'N/A';
    const reportedDate = account.AccountReportedDate || 'N/A';
  ...
  const getPaymentStatusStyle = (status: string | undefined | null) => {
    // Return default style if status is missing or not a string
  ...
  if (isCollapsed && isDisputeSaved) {
    return (
      <Card
        className="transition-all duration-700 shadow-sm border hover:shadow-md border-green-200 bg-green-50/50 rounded-lg"
        style={{
  ...
  return (
    <Card
      className={`
        ${isDisputeSaved ? 'border-2 border-green-300 bg-green-50' : hasAnyNegative ? 'border border-red-200 bg-red-50' : 'border border-gray-200'}
        transition-all duration-75 rounded-lg
      `}
      data-account-id={
        account['@CreditLiabilityID'] ||
        account['@_AccountNumber'] ||
    ...
    >
      <CardContent
        className={`
          pt-0 rounded-lg
          ${!hasAnyNegative ? 'px-6 pt-1 pb-2' : 'px-6 pt-6 pb-6'}
        `}
      >
                      ...
                      {isPublicRecord() ? (
                        ...
                        <>
                          ...
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            ...
                            <span
                              className={`text-sm ${getPaymentStatusStyle(getBureauDataByKey(account, 'TU').status)}`}
                            >
                              {getBureauDataByKey(account, 'TU').status}
                            </span>
                          </div>
                          <div className="flex justify-between border-b border-gray-100 py-2">
                          ...
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            ...
                            <span className="text-gray-900 text-sm">
                              ${getBureauDataByKey(account, 'TU').balance}
                            </span>
                          </div>
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            <span className="font-medium text-gray-700 text-sm">
                      ...
                      {isPublicRecord() ? (
                        ...
                        <>
                          ...
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            ...
                            <span
                              className={`text-sm ${getPaymentStatusStyle(getBureauDataByKey(account, 'EQ').status)}`}
                            >
                              {getBureauDataByKey(account, 'EQ').status}
                            </span>
                          </div>
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            <span className="font-medium text-gray-700 text-sm">Credit Limit:</span>
                          ...
                          <div className="flex justify-between border-b border-gray-100 py-2">
                            <span className="font-medium text-gray-700 text-sm">Balance:</span>
                            <span className="text-gray-900 text-sm">${getBureauDataByKey(account, 'EQ').balance}</span>
                          </div>
                          <div className="flex justify-between border-b border-gray-100 py-2">
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-1 gap-2 text-xs">
                      ...
                      <div className="flex justify-between border-b border-gray-100 py-2">
                        <span className="font-medium text-gray-700 text-sm">Payment Status:</span>
                        <span
                          className={`text-sm ${getPaymentStatusStyle(getBureauDataByKey(account, 'EX').status)}`}
                        >
                          {getBureauDataByKey(account, 'EX').status}
                        </span>
                      </div>
                      <div className="flex justify-between border-b border-gray-100 py-2">
                        <span className="font-medium text-gray-700 text-sm">Credit Limit:</span>
                      ...
                      <div className="flex justify-between border-b border-gray-100 py-2">
                        <span className="font-medium text-gray-700 text-sm">Balance:</span>
                        <span className="text-gray-900 text-sm">${getBureauDataByKey(account, 'EX').balance}</span>
                      </div>
                      <div className="flex justify-between border-b border-gray-100 py-2">
                        <span className="font-medium text-gray-700 text-sm">Monthly Payment:</span>
                        <span className="text-gray-900 text-sm">
...
{/* ─────────────────  Bureau Comparison Grid  ───────────────── */}
<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
  {[
    { key: 'TU',  label: 'TransUnion', color: 'text-cyan-700'   },
    { key: 'EQ',  label: 'Equifax',    color: 'text-red-600'    },
    { key: 'EX',  label: 'Experian',   color: 'text-blue-800'   },
  ].map(({ key, label, color }) => {
    console.log(`📋 Rendering ${account['@CreditLiabilityID']} → Bureau: ${key}`, getBureauDataByKey(account, key));
    const bData = getBureauDataByKey(account, key);
    const isReporting = bData.reporting;

    /* ─────────────── 1️⃣  REAL BUREAU CARD ─────────────── */
    if (isReporting) {
      return (
        <div key={key} className="flex flex-col">
          {/* 👉 mobile-only header */}
          <h3 className={`block md:hidden mb-2 font-bold ${color}`}>
            {label}
          </h3>
          ...
          <div
            className={`
              ...
              ${isDisputeSaved
                ...
                : bData.status === 'Negative' || (accountIsNegative && !bData.status)
          ...
          >
            ...
            <div className="mb-2 p-2 bg-yellow-100 border border-yellow-300 rounded text-xs">
              ...
              <p className="text-xs text-gray-400">Data Exists: {bData ? '✅' : '❌'}</p>
              <p className="text-xs text-gray-400">Reporting: {bData?.reporting ? '✅' : '❌'}</p>
            </div>
            
            {/* ----- top row: creditor & status dropdown ----- */}
            <div className="flex justify-between items-start mb-3">
              <h4 className="font-semibold text-black">{getCreditorName()}</h4>

              {/* 🔽 existing Select component */}
              <Select
                value={bData.status || (accountIsNegative ? 'Negative' : 'Positive')}
                onValueChange={(v) => setStatusForBureau(key, v)}   // ← your existing setter
              >
                <SelectTrigger
                  className={`
                    w-24 h-7 text-xs border-0 bg-transparent shadow-none
                    flex gap-1 items-center justify-center
                    [&>svg]:w-3 [&>svg]:h-3 [&>svg]:opacity-100
                    ${bData.status === 'Negative' || (accountIsNegative && !bData.status)
                      ? 'text-red-600 [&>svg]:text-red-600'
                      : 'text-green-700 [&>svg]:text-green-600'}
                  `}
                >
                  {bData.status === 'Negative' && <AlertTriangle className="w-3 h-3" />}
                  {bData.status === 'Positive' && <ThumbsUp       className="w-3 h-3" />}
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {statusOptions.map((s) => (
            ...
            <div className="space-y-2 text-xs grow">
              <InfoRow label="Account Type" value={getAccountField('@CreditBusinessType') || 'N/A'} />
              <InfoRow label="Account #"    value={(getAccountField('@_AccountNumber') || '').toString().replace(/\d(?=\d{4})/g, 'X') || 'N/A'} />
              <InfoRow label="Balance"      value={`$${bData?.balance || 'N/A'}`} />
              <InfoRow
                label="Status"
                value={bData?.status || 'N/A'}
                valueClass={getPaymentStatusStyle(bData?.status)}
              />
              <InfoRow label="Updated" value={bData?.lastUpdated || 'N/A'} />

              {/* ----- EXTRA DETAILS (toggle) ----- */}
              {shouldShowDetails && (
                <>
                  <InfoRow label="Date Opened" value={formatDate(bData?.openDate) || 'N/A'} />
                  <InfoRow label="Credit Limit" value={`$${getAccountField('@_CreditLimitAmount') || '0'}`} />
                  <InfoRow label="Payment Amount" value={`$${getAccountField('@_ActualPaymentAmount') || '0'}`} />
                  <div className="mt-3 pt-3 border-t border-gray-200">
                    <span className="text-xs font-medium text-gray-600">Payment History:</span>
                    <PaymentHistoryVisual
                      paymentPattern={getAccountField('@_PaymentHistoryProfile') || 'CCCCCCCCCCCCCCCCCCCCCCC'}
                      compact
                    />
                  </div>
                </>
    ...
    /* ─────────────── 2️⃣  NOT-REPORTING PLACEHOLDER ─────────────── */
    return (
      <div key={key} className="flex flex-col">
        {/* mobile header to keep heights identical */}
        <h3 className="block md:hidden mb-2 font-bold text-gray-400">
          {label}
        </h3>

        <div className="h-full flex flex-col items-center justify-center
                        border-2 border-dashed border-gray-300 rounded-lg
                        bg-gray-50 text-gray-400 shadow-sm py-8 px-4">
          <XCircle className="w-8 h-8 mb-2" />
          <span className="text-xs font-semibold">Not Reporting</span>
        </div>
      </div>
    );
  })}
                    ...
                    {!isTypingInstruction && !showCustomInstructionField ? (
                      ...
                      <div>
                        <div className="flex-between mb-2 min-h-[20px]">
                          ...
                          <button
                            onClick={() => {
                              // Reset only the instruction field
                              setCustomInstruction('');
                              setSelectedInstruction('');
                              setShowCustomInstructionField(false);
                              setIsTypingInstruction(false);
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      function proceedWithAccountSave() {
                        ...
                        setTimeout(() => {
                          // Check if this is a positive/closed account first
                          ...
                          const isInPositiveSection = cardElement ? !!cardElement.closest('[data-section="positive-accounts"]') : false;

                          if (isInPositiveSection) {
                            // POSITIVE ACCOUNT BEHAVIOR: Just collapse and stay in place with minimal scroll
                            setIsCollapsed(true);
                            
                            // Wait 1/2 second then scroll to 20px above same account
                            setTimeout(() => {
                              if (cardElement) {
                            ...
                            return; // Exit early - no negative account scrolling logic
                          }

                          // NEGATIVE ACCOUNT BEHAVIOR: Original logic
                          if (cardElement) {
                            const rect = cardElement.getBoundingClientRect();
