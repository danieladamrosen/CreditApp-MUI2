      ...
      setSelectedRecentInquiries((prev) => {
        ...
        if (wasEmpty && hasNewSelections && !selectedRecentReason && !selectedRecentInstruction) {
          ...
          setTimeout(() => {
            const isMobile = window.innerWidth < 768;
            if (isMobile) {
              console.log('MOBILE AUTO-SCROLL: Individual inquiry selection - executing mobile scroll with 500px offset');
              ...
              if (recentHeader) {
                ...
                console.log('MOBILE AUTO-SCROLL: Individual selection scrolling to:', targetScrollY, 'from header top:', rect.top);
                window.scrollTo({ top: targetScrollY, behavior: 'smooth' });
              }
            }
          }, 500);
      ...
      // If selecting after having none selected, trigger autotype and auto-scroll
        ...
        setTimeout(() => {
          ...
          if (isMobile && isSelecting) {
            console.log('MOBILE AUTO-SCROLL: Warning confirmation - executing mobile scroll with 500px offset');
            const recentHeader = document.querySelector('[data-testid="recent-inquiries-header"]');
            if (recentHeader) {
              const rect = recentHeader.getBoundingClientRect();
              // Find "Select All Score-Impact Items" button and scroll 20px below it
              ...
              const targetScrollY = button ? 
                window.pageYOffset + button.getBoundingClientRect().bottom + 20 :
                window.pageYOffset + rect.top + 230;
              console.log('MOBILE AUTO-SCROLL: Warning confirmation scrolling to:', targetScrollY, 'from header top:', rect.top);
              window.scrollTo({ top: targetScrollY, behavior: 'smooth' });
            }
          }
        }, 500);
                  ...
                  <div className="flex gap-2 h-full">
                    ...
                    <div className="flex-1 min-w-0 flex items-center">
                      <div className="w-full">
                        ...
                        <p
                          className={`text-xs flex items-center gap-1 ${isRecent ? 'text-yellow-600' : 'text-green-600'}`}
                        >
                          {isRecent ? (
                            <>
              ...
              <Button
                onClick={() => {
                  ...
                  if (isDisputeSavedState) {
                    ...
                    // Trigger same choreography based on inquiry type
                    if (isRecent) {
                      // Recent inquiries choreography
                      setTimeout(() => {
                          ...
                          setTimeout(() => {
                            setTimeout(() => {
                              ...
                              setTimeout(() => {
                                ...
                                const elements = document.querySelectorAll('*');
                                for (const element of elements) {
                                  if (element.textContent === '1' && element.classList.contains('circle-badge')) {
    ...
    <div className="space-y-3">
                  ...
                  <div className="flex flex-col justify-center">
                    ...
                    <div className="text-sm text-green-600 leading-4">
                      {(() => {
                        ...
                        const totalBureauDisputes = (disputedOlder > 0 ? getOlderBureauDisputeCount() : 0) + (disputedRecent > 0 ? getRecentBureauDisputeCount() : 0);
                        
                        // Show breakdown of types and bureau disputes
                        const parts = [];
                        if (disputedOlder > 0) parts.push(`${disputedOlder} older`);
                        if (disputedRecent > 0) parts.push(`${disputedRecent} recent`);
                        
                        if (parts.length > 0) {
                          return `${parts.join(' + ')} disputed • ${totalBureauDisputes} bureau disputes saved`;
                        }
      ...
      {!showCombinedCollapsedBox && !(isRecentDisputeSaved && isOlderDisputeSaved) && (
        <div className="mb-6">
          ...
          <div className="md:ml-9 text-center md:text-left">
      ...
      {!showCombinedCollapsedBox && (
        <>
          ...
          <Card
            className={`
            ${isOlderDisputeSaved ? 'border border-green-300 bg-green-50' : 'border-gray-200'}
            transition-all duration-75
          `}
          >
            <CardHeader
              className={`cursor-pointer hover:bg-gray-50 ${
                isOlderDisputeSaved ? 'hover:bg-green-100' : Object.values(selectedOlderInquiries).some(Boolean) ? 'hover:bg-red-75' : ''
              }`}
              ...
              <div className="flex justify-between items-center w-full">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className={`text-sm ${isOlderDisputeSaved ? 'text-green-600' : 'text-green-600'}`}>
                      {isOlderDisputeSaved
                        ? `${Object.values(selectedOlderInquiries).filter(Boolean).length} older disputed • ${getOlderBureauDisputeCount()} bureau disputes saved`
                        : 'No Impact To Score'}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-1">
                  {!isOlderDisputeSaved && (
                    <span className="text-sm text-gray-600">{getTotalOlderCount()} inquiries</span>
                  )}
                  {!showCombinedCollapsedBox && (
                    showOlderInquiries ? <ChevronUp /> : <ChevronDown />
                  )}
                </div>
              </div>
            </CardHeader>
            ...
            {showOlderInquiries && (
              <CardContent
                className={`
            pt-0 rounded-b-lg
            ${Object.values(selectedOlderInquiries).some(Boolean) && !isOlderDisputeSaved ? 'bg-red-50 border-red-200 border border-t-0' : ''}
      ...
      `}
          >
            <CardHeader
              ...
              className={`cursor-pointer hover:bg-gray-50 ${
                isRecentDisputeSaved ? 'hover:bg-green-100' : Object.values(selectedRecentInquiries).some(Boolean) ? 'hover:bg-red-75' : ''
              }`}
              ...
              <div className="flex justify-between items-center w-full">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className={`text-sm ${isRecentDisputeSaved ? 'text-green-600' : 'text-amber-600'}`}>
                      {isRecentDisputeSaved
                        ? `${Object.values(selectedRecentInquiries).filter(Boolean).length} recent disputed • ${getRecentBureauDisputeCount()} bureau disputes saved`
                ...
                <div className="flex items-center gap-1">
                  {!isRecentDisputeSaved && (
                    <span className="text-sm text-gray-600">{getTotalRecentCount()} inquiries</span>
                  )}
                  {!showCombinedCollapsedBox && (
                    showRecentInquiries ? <ChevronUp /> : <ChevronDown />
                  )}
                </div>
              </div>
            </CardHeader>
            ...
            {showRecentInquiries && (
              <CardContent
                className={`
            pt-5 rounded-b-lg
            ${Object.values(selectedRecentInquiries).some(Boolean) && !isRecentDisputeSaved ? 'bg-red-50 border-red-200 border border-t-0' : ''}
